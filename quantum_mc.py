...
def calc_euclidean_action(config, x_i, x_f, m, a, V_fn):
    ...
    #pot = 0.5 * (np.sum(V_fn(config[1:]) + V_fn(config[:-1]))
    #           + (V_fn(config[0]) + V_fn(x_i)) + (V_fn(x_f) + V_fn(config[-1]))) # boundary
    pot = 0.5 * np.sum(V_fn(config)) + 0.5 * V_fn(x_i) + 0.5 * V_fn(x_f)
    return a * (kin + pot)
...
norm = 1 / prop_vals[0] * analytic[0] # cheat by normalizing by the first analytical result
